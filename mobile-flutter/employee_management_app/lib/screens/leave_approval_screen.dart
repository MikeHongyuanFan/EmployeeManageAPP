...
class _LeaveApprovalScreenState extends State<LeaveApprovalScreen> {
  ...
  Widget _buildRequestsList() {
    ...
    final pendingRequests = _leaveRequests!.where((req) => req.status.toLowerCase() == 'pending').toList();
    final approvedRequests = _leaveRequests!.where((req) => req.status.toLowerCase() == 'approved').toList();
    final rejectedRequests = _leaveRequests!.where((req) => req.status.toLowerCase() == 'rejected').toList();
  ...
  Widget _buildRequestItem(LeaveRequest request, {bool showActions = true}) {
    return ListTile(
      title: Text(request.userName),
      subtitle: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          Text('${request.leaveType} Leave: ${request.startDate} to ${request.endDate}'),
  ...
  Future<void> _handleApproval(int requestId, String status, {String? reason}) async {
    try {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text('Leave request ${status.toUpperCase()}')),
      );
    } catch (e) {
      setState(() {
        _isLoading = false;
